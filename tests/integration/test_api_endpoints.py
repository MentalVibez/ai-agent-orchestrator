...
def mock_service_container():
    ...
    container.get_llm_manager.return_value = MagicMock()
...
def client(mock_service_container):
    """Create a test client with the mock container injected."""
...
class TestHealthEndpoint:
    ...
    def test_health_check_no_api_key_required(self, client):
...
class TestAgentsEndpoint:
    ...
    def test_list_agents_success(self, client, api_key_disabled):
        """Test listing agents endpoint."""
        response = client.get("/api/v1/agents", headers={"X-API-Key": "test-api-key"})
    ...
    def test_get_agent_detail_success(self, client, api_key_disabled):
        """Test getting agent details."""
        response = client.get(
            "/api/v1/agents/network_diagnostics", headers={"X-API-Key": "test-api-key"}
        )
    ...
    def test_get_agent_not_found(self, client, api_key_disabled):
        """Test getting non-existent agent."""
        response = client.get(
            "/api/v1/agents/nonexistent_agent", headers={"X-API-Key": "test-api-key"}
        )
...
class TestOrchestrateEndpoint:
    ...
    def test_orchestrate_task_validation_error(self, client, api_key_enabled):
