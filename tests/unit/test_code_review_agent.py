...
class TestCodeReviewAgent:
    ...
    def test_extract_issues(self, agent: CodeReviewAgent):
        ...
        assert len(issues) > 0
        assert any("vulnerability" in str(issue).lower() or "issue" in str(issue).lower() for issue in issues)
    ...
    def test_extract_recommendations(self, agent: CodeReviewAgent):
        ...
        assert any("recommend" in str(rec).lower() or "suggest" in str(rec).lower() for rec in recommendations)
