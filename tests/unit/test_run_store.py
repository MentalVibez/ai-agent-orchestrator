...
class TestRunStore:
    ...
    @patch("app.core.run_store.SessionLocal")
    def test_append_run_event_adds_event(self, mock_session_local):
        mock_db = MagicMock()
        mock_session_local.return_value = mock_db
        append_run_event("run-123", "status", {"status": "running"})
        mock_db.add.assert_called_once()
    ...
    @patch("app.core.run_store.SessionLocal")
    def test_get_run_events_returns_events_after_id(self, mock_session_local):
        mock_db = MagicMock()
        mock_session_local.return_value = mock_db
        mock_rows = [
        ...
        result = get_run_events("run-123", after_id=1)
        assert len(result) == 2
