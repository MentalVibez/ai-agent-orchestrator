...
class CostMetrics(BaseModel):
    ...
    cost_by_agent: Dict[str, float] = Field(
        default_factory=dict, description="Cost breakdown by agent"
    )
    cost_by_endpoint: Dict[str, float] = Field(
        default_factory=dict, description="Cost breakdown by endpoint"
    )
...
class CostMetricsResponse(BaseModel):
    ...
    recent_records: List[CostRecordResponse] = Field(
        default_factory=list, description="Recent cost records"
    )
