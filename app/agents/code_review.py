...
class CodeReviewAgent(BaseAgent):
    ...
    async def _collect_code_info(
        self, file_path: Optional[str], directory: str, focus_areas: list
    ) -> Dict[str, Any]:
        """
        Collect code information using tools.
        ...
        """
        code_info = {"files_analyzed": [], "structure": {}, "patterns": []}

        try:
            # List directory structure
            dir_result = await self.use_tool(
                "directory_list", {"directory": directory, "max_depth": 2}
            )
            ...
            if file_path:
                try:
                    file_result = await self.use_tool("file_read", {"file_path": file_path})
                ...
                for pattern in security_patterns:
                    try:
                        search_result = await self.use_tool(
                            "code_search",
                            {"pattern": pattern, "directory": directory, "file_pattern": "*.py"},
                        )
...
- Documentation gaps"""
    ...
    def _build_user_prompt(
        self, task: str, code_info: Dict[str, Any], context: Dict[str, Any]
    ) -> str:
...
5. Best practices suggestions"""
    ...
    def _identify_review_type(self, task: str, focus_areas: list) -> str:
        ...
        if any(
            keyword in task_lower for keyword in ["security", "vulnerability", "exploit", "attack"]
        ):
    ...
    def _extract_issues(self, response: str) -> list:
        ...
        for i, line in enumerate(lines):
            if any(
                ...
                for keyword in ["vulnerability", "issue", "problem", "bug", "risk"]
            ):
    ...
    def _extract_recommendations(self, response: str) -> list:
        ...
        for i, line in enumerate(lines):
            if any(
                ...
                recommendations.append({"type": "recommendation", "description": line.strip()})
